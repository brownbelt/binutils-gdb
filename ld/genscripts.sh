...
LD_FLAG=readonly
( echo "/* Script for -z readonly: generate readonly segment */"
  . ${CUSTOMIZER_SCRIPT}
  . ${srcdir}/scripttempl/${SCRIPT_NAME}.sc
...
if test -n "$GENERATE_COMBRELOC_SCRIPT"; then
  ...
  ( echo "/* Script for -z combreloc -z readonly: combine and sort reloc sections */"
    . ${CUSTOMIZER_SCRIPT}
    . ${srcdir}/scripttempl/${SCRIPT_NAME}.sc
  ...
  ( echo "/* Script for -z combreloc -z now -z relro -z readonly: combine and sort reloc sections */"
    . ${CUSTOMIZER_SCRIPT}
    . ${srcdir}/scripttempl/${SCRIPT_NAME}.sc
...
if test -n "$GENERATE_SHLIB_SCRIPT"; then
  ...
  LD_FLAG=sharedreadonly
  (
    echo "/* Script for ld --shared -z readonly: link shared library */"
    . ${CUSTOMIZER_SCRIPT}
    . ${srcdir}/scripttempl/${SCRIPT_NAME}.sc
  ...
  if test -n "$GENERATE_COMBRELOC_SCRIPT"; then
    ...
    rm -f ${COMBRELOC}
    LD_FLAG=csharedreadonly
    COMBRELOC=ldscripts/${EMULATION_NAME}.xsce.tmp
    ( echo "/* Script for --shared -z combreloc -z readonly: shared library, combine & sort relocs */"
      . ${CUSTOMIZER_SCRIPT}
      . ${srcdir}/scripttempl/${SCRIPT_NAME}.sc
    ...
    rm -f ${COMBRELOC}
    LD_FLAG=wsharedreadonly
    COMBRELOC=ldscripts/${EMULATION_NAME}.xswe.tmp
    ( echo "/* Script for --shared -z combreloc -z now -z relro -z readonly: shared library, combine & sort relocs */"
      . ${CUSTOMIZER_SCRIPT}
      . ${srcdir}/scripttempl/${SCRIPT_NAME}.sc
...
if test -n "$GENERATE_PIE_SCRIPT"; then
  ...
  LD_FLAG=piereadonly
  (
    echo "/* Script for ld -pie -z readonly: link position independent executable */"
    . ${CUSTOMIZER_SCRIPT}
    . ${srcdir}/scripttempl/${SCRIPT_NAME}.sc
  ...
  if test -n "$GENERATE_COMBRELOC_SCRIPT"; then
    ...
    rm -f ${COMBRELOC}
    LD_FLAG=cpiereadonly
    COMBRELOC=ldscripts/${EMULATION_NAME}.xdce.tmp
    ( echo "/* Script for -pie -z combreloc -z readonly: position independent executable, combine & sort relocs */"
      . ${CUSTOMIZER_SCRIPT}
      . ${srcdir}/scripttempl/${SCRIPT_NAME}.sc
    ...
    rm -f ${COMBRELOC}
    LD_FLAG=wpiereadonly
    COMBRELOC=ldscripts/${EMULATION_NAME}.xdwe.tmp
    ( echo "/* Script for -pie -z combreloc -z now -z relro -z readonly: position independent executable, combine & sort relocs */"
      . ${CUSTOMIZER_SCRIPT}
      . ${srcdir}/scripttempl/${SCRIPT_NAME}.sc
